<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <link rel="stylesheet" href="../style/home.css" />
  <title>LeBonCôté</title>
</head>

<body>
  <header>
    <a href="#" class="logo"><span>LeBonCôté</span></a>
    <ul class="navbar">
      <li><a href="index.php">Accueil</a></li>
      <li><a href="../recherche/recherche.php">Recherche</a></li>
      <li><a href="../moncompte/mon_compte.php">Mon Compte</a></li>
      <li><a href="../inscription/inscription.php">Inscription</a></li>
      <a href="../publication/publication.php" class="btn-pub">Publier une annonce</a>
    </ul>
  </header>
  <section class="banniere" id="banniere">
    <div class="contenu">
      <h3>Des millions de petites annonces</h3>
      <p>Retrouvez une grande variété d'articles d'occasion</p>
      <a href="../connexion/connexion.php" class="btn1">Se connecter</a>
    </div>
  </section>
  <section class="apropos" id="apropos">
    <div>
      <div>
        <h2><span>A</span> Propos de nous</h2>
        <p style="color: black; font-size: 20px">LeBonCôté est la meilleure plateforme pour la vente d'occasion. Trouvez
          des offres incroyables sur une variété de produits.</p>
      </div>
    </div>
  </section>
  <footer>
    <p>&copy; 2024 LeBonCôté</p>
  </footer>
</body>

</html>